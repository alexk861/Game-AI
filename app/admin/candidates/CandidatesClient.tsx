      ...
      <div className="max-w-7xl mx-auto">
        ...
        {statusFilter === 'intelligence' && intelligenceData ? (
          <div className="space-y-8 animate-fade-in">
            {/* Audience Analytics & Real-Time Telemetry Dashboard */}
            {intelligenceData.audience_stats && (
              <div className="bg-slate-900 border border-slate-800 text-white rounded-2xl p-6 shadow-xl relative overflow-hidden">
                ...
                <h2 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-yellow-300 flex items-center gap-2 mb-6">
                  <span>📊</span> Audience Analytics & Real-Time Telemetry
                </h2>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 relative z-10">
                  <div className="bg-slate-800/40 rounded-xl p-5 border border-slate-700/40 backdrop-blur-sm">
                    <span className="text-xs text-slate-400 uppercase font-semibold tracking-wider block">Total Decisions (Guesses) Logged</span>
                    <div className="text-4xl font-extrabold text-amber-400 mt-2">
                      {intelligenceData.audience_stats.total_plays.toLocaleString()}
                    </div>
                    <div className="text-xs text-slate-500 mt-2 flex justify-between">
                      <span>Real: {intelligenceData.audience_stats.guesses_real.toLocaleString()}</span>
                      <span>AI: {intelligenceData.audience_stats.guesses_ai.toLocaleString()}</span>
                    </div>
                  </div>

                  <div className="bg-slate-800/40 rounded-xl p-5 border border-slate-700/40 backdrop-blur-sm">
                    <span className="text-xs text-slate-400 uppercase font-semibold tracking-wider block">Total Estimated Games Completed</span>
                    <div className="text-4xl font-extrabold text-indigo-400 mt-2">
                      {Math.round(intelligenceData.audience_stats.total_plays / 5).toLocaleString()}
                    </div>
                    <p className="text-xs text-slate-500 mt-2">
                      Calculated as Total Guesses / 5.
                    </p>
                  </div>

                  <div className="bg-slate-800/40 rounded-xl p-5 border border-slate-700/40 backdrop-blur-sm">
                    <span className="text-xs text-slate-400 uppercase font-semibold tracking-wider block">Player Accuracy</span>
                    <div className="text-4xl font-extrabold text-green-400 mt-2">
                      {(intelligenceData.audience_stats.accuracy_rate * 100).toFixed(1)}%
                    </div>
                    <div className="mt-2 h-1.5 w-full bg-slate-950 rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-green-500 rounded-full transition-all duration-500" 
                        style={{ width: `${intelligenceData.audience_stats.accuracy_rate * 100}%` }}
                      ></div>
                    </div>
                  </div>

                  <div className="bg-slate-800/40 rounded-xl p-5 border border-slate-700/40 backdrop-blur-sm">
                    <span className="text-xs text-slate-400 uppercase font-semibold tracking-wider block">AI Suspicion Bias</span>
                    <div className="text-4xl font-extrabold text-purple-400 mt-2">
                      {(intelligenceData.audience_stats.ai_bias_ratio * 100).toFixed(1)}%
                    </div>
                    <div className="mt-2 h-1.5 w-full bg-slate-950 rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-purple-500 rounded-full transition-all duration-500" 
                        style={{ width: `${intelligenceData.audience_stats.ai_bias_ratio * 100}%` }}
                      ></div>
                    </div>
                  </div>

                  <div className="bg-slate-800/40 rounded-xl p-5 border border-slate-700/40 backdrop-blur-sm">
                    <span className="text-xs text-slate-400 uppercase font-semibold tracking-wider block">Audience Fooled Count</span>
                    <div className="text-4xl font-extrabold text-red-400 mt-2">
                      {intelligenceData.audience_stats.total_fooled.toLocaleString()}
                    </div>
                    <p className="text-xs text-slate-500 mt-2">
                      Incorrect decisions made against active challenge images.
                    </p>
                  </div>
                </div>

                {/* Most Fooled Active Challenges */}
                <div className="mt-8 relative z-10">
                  <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-300 mb-4 flex items-center gap-1.5">
                    <span>🔥</span> Most Fooled Challenges (Top Player Trap Points)
                  </h3>
                  {intelligenceData.audience_stats.top_fooled.length === 0 ? (
                    <p className="text-xs text-slate-500 italic bg-slate-950/30 p-4 rounded-xl border border-slate-800">No telemetry plays collected yet.</p>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                      {intelligenceData.audience_stats.top_fooled.slice(0, 5).map((ch, idx) => (
                        <div key={ch.id} className="bg-slate-950/40 border border-slate-800/80 rounded-xl overflow-hidden p-3 flex flex-col justify-between hover:border-amber-500/30 transition-colors">
                          <div className="relative aspect-[16/10] w-full bg-slate-800 rounded-lg overflow-hidden border border-slate-800">
                            <Image
                              src={ch.image_url}
                              alt=""
                              fill
                              ...
                              sizes="120px"
                            />
                            <div className="absolute top-2 left-2 bg-slate-950/80 backdrop-blur-md px-1.5 py-0.5 rounded text-[8px] font-mono text-slate-300">
                              #{idx + 1}
                            </div>
                            <div className={`absolute bottom-2 right-2 px-1.5 py-0.5 rounded text-[8px] font-mono uppercase font-bold ${
                              ch.answer === 'real' ? 'bg-blue-900/80 text-blue-200' : 'bg-purple-900/80 text-purple-200'
                            }`}>
                              {ch.answer}
                            </div>
                          </div>
                          ...
                          <div className="mt-3 flex flex-col gap-1">
                            <span className="text-[10px] font-mono text-slate-500">Set: {ch.set_date} (Slot {ch.set_order})</span>
                            <p className="text-[11px] text-slate-300 line-clamp-2 italic" title={ch.context_short}>
                              &ldquo;{ch.context_short}&rdquo;
                            </p>
                          </div>

                          <div className="mt-3 bg-slate-900 p-2 rounded-lg text-xs font-mono border border-slate-800 flex flex-col gap-1.5">
                            <div className="flex justify-between">
                              <span className="text-slate-500 text-[9px]">Fooled Rate</span>
                              <span className="font-bold text-red-400">{(ch.fooled_rate * 100).toFixed(0)}%</span>
                            </div>
                            <div className="h-1 w-full bg-slate-950 rounded-full overflow-hidden">
                              <div className="h-full bg-red-500 rounded-full" style={{ width: `${ch.fooled_rate * 100}%` }}></div>
                            </div>
                            <div className="flex justify-between text-[9px] text-slate-500 mt-1">
                              <span>Fooled: {ch.fooled_count}</span>
                              <span>Total: {ch.total_guesses}</span>
                            </div>
                          </div>
                        </div>
                      ))}
