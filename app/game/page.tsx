...
export default function Home({ challengeDate }: { challengeDate?: string } = {}) {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (!hasStartedToday()) {
        // Bypass welcome screen: Mark today as started and load immediately
        const timingState = markTodayStarted();
        setSessionStartedAt(timingState.todayStartedAt);
        setElapsedMs(0);
        setCompletionMs(null);
        void loadDailySet(timingState, false);
        return;
      }
  ...
  if (phase === 'entry') {
    return (
      <>
        <WelcomeScreen onBegin={handleBegin} zenMode={zenMode} setZenMode={setZenMode} isChallengePlay={isChallengePlay} />
        ...
        <main className="flex h-[100dvh] flex-col items-center justify-center bg-background cinematic-bg px-8">
          ...
          {isChallengePlay ? (
            <div className="relative z-10 flex flex-col items-center max-w-xs text-center font-sans">
              <div className="font-mono text-[9px] uppercase tracking-[0.2em] text-muted/50 mb-6">
                // Set Unavailable
              </div>
              ...
              <Link
                href="/"
                className="min-w-[160px] bg-[#f0ece9] text-[#1f2126] font-sans text-xs uppercase tracking-[0.12em] font-medium py-3.5 px-6 text-center rounded-[3px] transition-all hover:bg-[#f0ece9]/90 active:scale-[0.985] cursor-pointer"
              >
                Play today’s set
              </Link>
            </div>
