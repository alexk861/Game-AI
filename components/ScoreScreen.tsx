  ...
  return (
    <div className="absolute inset-0 z-40 flex flex-col items-center justify-center archive-bg px-6">
      ...
      <div className="archive-panel p-8 flex flex-col items-center w-full max-w-sm">
        <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-muted/95 mb-7 fade-in">
          {copy.results.label}
        </div>

        <div className="score-reveal">
          <div className="text-6xl font-normal font-mono tracking-normal">
            {score}<span className="text-2xl text-muted/90">/5</span>
          </div>
        </div>
          ...
          {results.map((r, i) => (
            <div
              ...
              className={`w-10 h-10 flex items-center justify-center text-xs font-mono border ${
                r.guess === 'timeout'
                  ? 'border-outline-variant bg-surface-container text-muted/90'
                  : r.correct
        ...
        {streak > 0 && (
          <div className="mt-6 text-[10px] font-mono text-muted/95 uppercase tracking-[0.16em] fade-in" style={{ animationDelay: '0.7s' }}>
      ...
      <div className="absolute bottom-6 text-[10px] text-muted/90 font-mono uppercase tracking-[0.15em] fade-in" style={{ animationDelay: '1.1s' }}>
        {setDate} / uncanny.app
      </div>
    </div>
  );
